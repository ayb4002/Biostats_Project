...
Patient_Progression <- ggplot(plot_data,
                         aes(axis1 = STAGE,
                             axis2 = BESTRESP,
                             y = COUNT)) +
...
```{r}
ggarrange(Patient_Progression, Best_Response, 
          labels = c("A", "B"),
          ncol = 1, nrow = 1)
```
...
check if 2 arms have the same amount of male & female in different arms 
H0: The 2 arms have the same amount of male & female in different arms 
H1: The 2 arms do not have the same amount of male & female in different arms 

```{r, echo=FALSE}
prop.table(table(dat$SEX, dat$ARMCD))

results_1 <- ggplot(dat, aes(ARMCD, fill = SEX)) + 
              geom_bar(stat = "count")+  
              theme_bw() +
              labs(title = "Bar Plot of Sex in 2 Arms") +
              theme(plot.title = element_text(hjust = 0.5))
...
check if 2 arms have the same amount of RACE in different arms 
H0: The 2 arms have the same amount of RACE in different arms 
H1: The 2 arms do not have the same amount of RACE in different arms 
```{r, echo=FALSE}
prop.table(table(dat$RACE, dat$ARMCD))

results_2 <- ggplot(dat, aes(ARMCD, fill = RACE)) + 
              geom_bar(stat = "count")+
              theme_bw() +
              labs(title = "Bar Plot of Race in 2 Arms") +
              theme(plot.title = element_text(hjust = 0.5))
...
results_3 <- ggplot(dat, aes(ARMCD, fill = ETHNIC)) + 
              geom_bar(stat = "count")+
              theme_bw() +
              labs(title = "Bar Plot of ETHNIC in 2 Arms") +
              theme(plot.title = element_text(hjust = 0.5))
...
results_4 <- ggplot(dat, aes(ARMCD, fill = STAGE)) + 
              geom_bar(stat = "count") +
              theme_bw() +
              labs(title = "Bar Plot of STAGE in 2 Arms") +
              theme(plot.title = element_text(hjust = 0.5))
...
results_5 <- dat_grade %>% ggplot(aes(y = HISTGRDE, fill = ARMCD)) +
              geom_bar(stat = "count") +
              theme_bw() +
              labs(title = "Bar Plot of Histological Grade in 2 Arms") +
              theme(plot.title = element_text(hjust = 0.5))
...
results_6 <- ggplot(dat, aes(tumor_size_change, fill = ARMCD)) + 
              geom_density(alpha=0.5) +
              theme_bw() +
              labs(title = "Density Plot of Tumor Size Change in 2 Arms") +
              theme(plot.title = element_text(hjust = 0.5))
...
results_7 <- ggplot(dat, aes(ARMCD, fill = BESTRESP)) + 
              geom_bar(stat = "count") +
              theme_bw() +
              labs(title = "Bar Plot of Best Response in 2 Arms") +
              theme(plot.title = element_text(hjust = 0.5))
...
```{r}
ggarrange(results_1, results_2, results_3, results_4, results_5, results_6, results_7,
          #labels = c("A", "B"),
          ncol = 1, nrow = 1)
```
...
1. If a patient belongs to Arm B, it will decrease the log odds of getting a tumor response by 0.554; so arm A (DOX+EVO) gives more tumor response
2. If a patient's race is BLACK OR AFRICAN AMERICAN, it will decrease the log odds of getting a tumor response by 3.17

We can also interpret the results suing Odds ratio:

1. If a patient belongs to Arm B, the odds of getting a tumor response by 0.554; so arm A (DOX+EVO) gives more tumor response
2. If a patient's race is BLACK OR AFRICAN AMERICAN, it will decrease the log odds of getting a tumor response by 3.17
